...
func processJsonRpc(in io.Reader, out io.Writer, conn *_root.Connection) error {
	...
	if err = conn.Context(func (context *_root.API)error {
		...
		case "IsRepoConflicted":
			...
			if len(request.Params)!=2 {
				err = fmt.Errorf(
					"Expected %d parameters, but got %d in call to %s", 2, len(request.Params), "IsRepoConflicted")
				errorJsonRpc(out, request.Id, JSONRPC_ERROR_INVALID_PARAMS, err, nil)
				return err
			}
			...
			if err = func() (err error) {
				...
				defer func() {
					...
					result[0],result[1] = context.IsRepoConflicted(
		args.REPOOWNER,
		args.REPONAME,
		...
		case "ListWatchers":
			...
			if len(request.Params)!=2 {
				err = fmt.Errorf(
					"Expected %d parameters, but got %d in call to %s", 2, len(request.Params), "ListWatchers")
				errorJsonRpc(out, request.Id, JSONRPC_ERROR_INVALID_PARAMS, err, nil)
				return err
			}
			...
			if err = func() (err error) {
				...
				defer func() {
					...
					result[0],result[1] = context.ListWatchers(
		args.REPOOWNER,
		args.REPONAME,
		...
		case "ListCommits":
			...
			if len(request.Params)!=2 {
				err = fmt.Errorf(
					"Expected %d parameters, but got %d in call to %s", 2, len(request.Params), "ListCommits")
				errorJsonRpc(out, request.Id, JSONRPC_ERROR_INVALID_PARAMS, err, nil)
				return err
			}
			...
			if err = func() (err error) {
				...
				defer func() {
					...
					result[0],result[1] = context.ListCommits(
		args.REPOOWNER,
		args.REPONAME,
		...
		case "ListWatched":
			...
			if err = func() (err error) {
				...
				defer func() {
					...
					result[0],result[1] = context.ListWatched(
		...
		case "MergedFileGit":
			...
			if len(request.Params)!=3 {
				err = fmt.Errorf(
					"Expected %d parameters, but got %d in call to %s", 3, len(request.Params), "MergedFileGit")
				errorJsonRpc(out, request.Id, JSONRPC_ERROR_INVALID_PARAMS, err, nil)
				return err
			}
			...
			if err = func() (err error) {
				...
				defer func() {
					...
					result[0],result[1],result[2] = context.MergedFileGit(
		args.REPOOWNER,
		args.REPONAME,
		args.PATH,
		...
		case "ReadFileData":
			...
			if len(request.Params)!=4 {
				err = fmt.Errorf(
					"Expected %d parameters, but got %d in call to %s", 4, len(request.Params), "ReadFileData")
				errorJsonRpc(out, request.Id, JSONRPC_ERROR_INVALID_PARAMS, err, nil)
				return err
			}
			...
			if err = json.Unmarshal(request.Params[2], &args.VERSION);
				nil!=err {
				errorJsonRpc(out, request.Id, JSONRPC_ERROR_INVALID_PARAMS, fmt.Errorf(
					"Unable to decode JSON param %d: %s", 2+1, err.Error()), nil)
				return err
			...
			if err = json.Unmarshal(request.Params[3], &args.PATH);
				nil!=err {
				errorJsonRpc(out, request.Id, JSONRPC_ERROR_INVALID_PARAMS, fmt.Errorf(
					"Unable to decode JSON param %d: %s", 3+1, err.Error()), nil)
				return err
			...
			if err = func() (err error) {
				...
				defer func() {
					...
					result[0],result[1] = context.ReadFileData(
		args.REPOOWNER,
		args.REPONAME,
		args.VERSION,
		args.PATH,
		...
		case "WriteAndStageFile":
			...
			if len(request.Params)!=4 {
				err = fmt.Errorf(
					"Expected %d parameters, but got %d in call to %s", 4, len(request.Params), "WriteAndStageFile")
				errorJsonRpc(out, request.Id, JSONRPC_ERROR_INVALID_PARAMS, err, nil)
				return err
			}
			...
			if err = func() (err error) {
				...
				defer func() {
					...
					result[0],result[1] = context.WriteAndStageFile(
		args.REPOOWNER,
		args.REPONAME,
		args.PATH,
		args.DATA,
		...
		case "RemoveAndStageFile":
			...
			if len(request.Params)!=3 {
				err = fmt.Errorf(
					"Expected %d parameters, but got %d in call to %s", 3, len(request.Params), "RemoveAndStageFile")
				errorJsonRpc(out, request.Id, JSONRPC_ERROR_INVALID_PARAMS, err, nil)
				return err
			}
			...
			if err = func() (err error) {
				...
				defer func() {
					...
					result[0] = context.RemoveAndStageFile(
		args.REPOOWNER,
		args.REPONAME,
		args.PATH,
		...
		case "FileExistsOurHeadTheirHead":
			...
			if len(request.Params)!=3 {
				err = fmt.Errorf(
					"Expected %d parameters, but got %d in call to %s", 3, len(request.Params), "FileExistsOurHeadTheirHead")
				errorJsonRpc(out, request.Id, JSONRPC_ERROR_INVALID_PARAMS, err, nil)
				return err
			}
			...
			if err = func() (err error) {
				...
				defer func() {
					...
					result[0],result[1],result[2] = context.FileExistsOurHeadTheirHead(
		args.REPOOWNER,
		args.REPONAME,
		args.PATH,
		...
		case "IsOurHeadInWd":
			...
			if len(request.Params)!=3 {
				err = fmt.Errorf(
					"Expected %d parameters, but got %d in call to %s", 3, len(request.Params), "IsOurHeadInWd")
				errorJsonRpc(out, request.Id, JSONRPC_ERROR_INVALID_PARAMS, err, nil)
				return err
			}
			...
			if err = func() (err error) {
				...
				defer func() {
					...
					result[0],result[1] = context.IsOurHeadInWd(
		args.REPOOWNER,
		args.REPONAME,
		args.PATH,
		...
		case "SaveOurHeadToWd":
			...
			if len(request.Params)!=3 {
				err = fmt.Errorf(
					"Expected %d parameters, but got %d in call to %s", 3, len(request.Params), "SaveOurHeadToWd")
				errorJsonRpc(out, request.Id, JSONRPC_ERROR_INVALID_PARAMS, err, nil)
				return err
			}
			...
			if err = func() (err error) {
				...
				defer func() {
					...
					result[0] = context.SaveOurHeadToWd(
		args.REPOOWNER,
		args.REPONAME,
		args.PATH,
		...
		case "SaveTheirHeadToWd":
			...
			if len(request.Params)!=3 {
				err = fmt.Errorf(
					"Expected %d parameters, but got %d in call to %s", 3, len(request.Params), "SaveTheirHeadToWd")
				errorJsonRpc(out, request.Id, JSONRPC_ERROR_INVALID_PARAMS, err, nil)
				return err
			}
			...
			if err = func() (err error) {
				...
				defer func() {
					...
					result[0] = context.SaveTheirHeadToWd(
		args.REPOOWNER,
		args.REPONAME,
		args.PATH,
		...
		case "SearchForFiles":
			...
			if len(request.Params)!=3 {
				err = fmt.Errorf(
					"Expected %d parameters, but got %d in call to %s", 3, len(request.Params), "SearchForFiles")
				errorJsonRpc(out, request.Id, JSONRPC_ERROR_INVALID_PARAMS, err, nil)
				return err
			}
			...
			if err = json.Unmarshal(request.Params[2], &args.FILEREGEX);
				nil!=err {
				errorJsonRpc(out, request.Id, JSONRPC_ERROR_INVALID_PARAMS, fmt.Errorf(
					"Unable to decode JSON param %d: %s", 2+1, err.Error()), nil)
				return err
			...
			if err = func() (err error) {
				...
				defer func() {
					...
					result[0],result[1],result[2] = context.SearchForFiles(
		args.REPOOWNER,
		args.REPONAME,
		args.FILEREGEX,
		...
		case "UpdateFileBinary":
			...
			if len(request.Params)!=4 {
				err = fmt.Errorf(
					"Expected %d parameters, but got %d in call to %s", 4, len(request.Params), "UpdateFileBinary")
				errorJsonRpc(out, request.Id, JSONRPC_ERROR_INVALID_PARAMS, err, nil)
				return err
			}
			...
			if err = json.Unmarshal(request.Params[3], &args.CONTENTB64);
				nil!=err {
				errorJsonRpc(out, request.Id, JSONRPC_ERROR_INVALID_PARAMS, fmt.Errorf(
					"Unable to decode JSON param %d: %s", 3+1, err.Error()), nil)
				return err
			...
			if err = func() (err error) {
				...
				defer func() {
					...
					result[0] = context.UpdateFileBinary(
		args.REPOOWNER,
		args.REPONAME,
		args.PATH,
		args.CONTENTB64,
