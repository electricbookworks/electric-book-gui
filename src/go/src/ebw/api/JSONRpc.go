...
func (rpc *API) DeleteFile(repo, path string) error {
	return git.DeleteFile(rpc.GetContext(), rpc.Client, rpc.User, repo, path)
}
...
func (rpc *API) ListFiles(repo, pathregex string) ([]string, error) {
	return git.ListFiles(rpc.GetContext(), rpc.Client, rpc.User, repo, pathregex)
}

// GetFile gets the contents of the file at path in the repo.
func (rpc *API) GetFile(repo, path string) ([]byte, error) {
	return git.GetFile(rpc.GetContext(), rpc.Client, rpc.User, repo, path)
}

func (rpc *API) GetFileString(repo, path string) (string, error) {
	raw, err := git.GetFile(rpc.GetContext(), rpc.Client, rpc.User, repo, path)
	return string(raw), err
}

func (rpc *API) UpdateFile(repo, path, content string) error {
	return git.UpdateFile(rpc.GetContext(), rpc.Client, rpc.User, repo, path, []byte(content))
}

func (rpc *API) ListPullRequests(repo string) ([]*github.PullRequest, error) {
	return git.ListPullRequests(rpc.GetContext(), rpc.Client, rpc.User, repo)
}

func (rpc *API) PullRequestDiffList(repo, sha, regexp string) ([]*git.PullRequestDiff, error) {
	return git.PullRequestDiffList(rpc.GetContext(), rpc.Client, rpc.User, repo, sha, regexp)
}

func (rpc *API) PullRequestVersions(repo, remoteUrl, remoteSha, filePath string) (string, string, error) {
	return git.PullRequestVersions(rpc.GetContext(), rpc.Client, rpc.User, repo, remoteUrl, remoteSha, filePath)
}

func (rpc *API) PullRequestUpdate(repo, remoteSHA, filePath string, data string) error {
	return git.PullRequestUpdate(rpc.GetContext(), rpc.Client, rpc.User, repo, remoteSHA, filePath, []byte(data))
}
