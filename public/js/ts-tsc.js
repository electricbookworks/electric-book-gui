...
(function (G) {
    function r(d, b, c) {
        var h = 0, a = [], f = 0, g, m, k, e, l, p, q, t, w = !1, n = [], u = [], v, r = !1;
        c = c || {};
        g = c.encoding || "UTF8";
        v = c.numRounds || 1;
        ...
        if ("SHA-1" === d)
            l = 512, p = z, q = H, e = 160, t = function (a) { return a.slice(); };
        else
        ...
        this.update = function (b) { var e, g, c, d = 0, q = l >>> 5; e = k(b, a, f); b = e.binLen; g = e.value; e = b >>> 5; for (c = 0; c < e; c += q)
            d + l <= b && (m = p(g.slice(c, c + q), m), d += l); h += d; a = g.slice(d >>> 5); f = b % l; r = !0; };
        this.getHash = function (b, g) {
            ...
            switch (b) {
                case "HEX":
                    c = function (a) { return C(a, e, l); };
                    break;
                case "B64":
                    c = function (a) { return D(a, e, l); };
                    break;
                case "BYTES":
                    c = function (a) { return E(a, e); };
                    break;
                case "ARRAYBUFFER":
                    try {
                        k = new ArrayBuffer(0);
        ...
        this.getHMAC = function (b, g) {
            ...
            switch (b) {
                case "HEX":
                    c = function (a) { return C(a, e, n); };
                    break;
                case "B64":
                    c = function (a) { return D(a, e, n); };
                    break;
                case "BYTES":
                    c = function (a) { return E(a, e); };
                    break;
    ...
    function C(d, b, c) { var h = ""; b /= 8; var a, f; for (a = 0; a < b; a += 1)
        f = d[a >>> 2] >>> 8 * (3 + a % 4 * -1), h += "0123456789abcdef".charAt(f >>> 4 & 15) + "0123456789abcdef".charAt(f & 15); return c.outputUpper ? h.toUpperCase() : h; }
    function D(d, b, c) {
        var h = "", a = b / 8, f, g, m;
        for (f = 0; f < a; f += 3)
            for (g = f + 1 < a ? d[f + 1 >>> 2] : 0, m = f + 2 < a ? d[f + 2 >>> 2] : 0, m = (d[f >>> 2] >>> 8 * (3 + f % 4 * -1) & 255) << 16 | (g >>> 8 * (3 + (f + 1) % 4 * -1) & 255) << 8 | m >>> 8 * (3 + (f + 2) % 4 * -1) & 255, g = 0; 4 > g; g += 1)
                8 * f + 6 * g <= b ? h += "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(m >>>
                    6 * (3 - g) & 63) : h += c.b64Pad;
        return h;
    }
    function E(d, b) { var c = "", h = b / 8, a, f; for (a = 0; a < h; a += 1)
        f = d[a >>> 2] >>> 8 * (3 + a % 4 * -1) & 255, c += String.fromCharCode(f); return c; }
    function F(d, b) { var c = b / 8, h, a = new ArrayBuffer(c), f; f = new Uint8Array(a); for (h = 0; h < c; h += 1)
        f[h] = d[h >>> 2] >>> 8 * (3 + h % 4 * -1) & 255; return a; }
    function B(d) {
        ...
        switch (d) {
            ...
            case "TEXT":
                c = function (c, a, f) {
                    ...
                    return { value: a, binLen: 8 * k + f };
                };
                break;
            case "B64":
                c = function (b, a, f) {
                    var c = 0, d, k, e, l, p, q, n;
            ...
            case "BYTES":
                c = function (b, a, c) {
                    ...
                    return { value: a, binLen: 8 * b.length + c };
                };
                break;
            case "ARRAYBUFFER":
                try {
                ...
                c = function (b, a, c) { var d, m, k, e, l; a = a || [0]; c = c || 0; m = c >>> 3; l = new Uint8Array(b); for (d = 0; d < b.byteLength; d += 1)
                    e = d + m, k = e >>> 2, a.length <= k && a.push(0), a[k] |= l[d] << 8 * (3 + e % 4 * -1); return { value: a, binLen: 8 * b.byteLength + c }; };
                break;
            default: throw Error("format must be HEX, TEXT, B64, BYTES, or ARRAYBUFFER");
        }
        return c;
    }
    function n(d, b) { return d << b | d >>> 32 - b; }
    function u(d, b) { var c = (d & 65535) + (b & 65535); return ((d >>> 16) + (b >>> 16) + (c >>> 16) & 65535) << 16 | c & 65535; }
    function y(d, b, c, h, a) { var f = (d & 65535) + (b & 65535) + (c & 65535) + (h & 65535) + (a & 65535); return ((d >>> 16) + (b >>> 16) + (c >>> 16) + (h >>> 16) + (a >>> 16) + (f >>> 16) & 65535) << 16 | f & 65535; }
    function x(d) { var b = []; if ("SHA-1" === d)
        b = [1732584193, 4023233417, 2562383102, 271733878, 3285377520];
    else
        throw Error("No SHA variants supported"); return b; }
    function z(d, b) {
        var c = [], h, a, f, g, m, k, e;
        h = b[0];
        a = b[1];
...
//# sourceMappingURL=ts-tsc.js.map